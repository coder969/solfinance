...
<%
    IList<CarOrder> orderlist = ViewData["orderlist"] as IList<CarOrder>;
...
%>
<div class="wz">&nbsp;&nbsp;&nbsp;您当前所在的位置： <a href="/">风采旅行社首页</a> » <a href="/Member/Main">会员中心首页</a> »租车订单</div>
<div class="page">
  ...
  <%if(orderlist != null && orderlist.Count > 0)
  {
    foreach(var item in orderlist)
    {
      ...
      <tr class="urtabletd">
        ...
        <td class="urtabletdr"><a><%:item.cname %></a></td>
        <td><%:item.bookdate %></td>
        <td><%:item.lxr %></td>
        <td><%:item.phone %></td>
...
<%if (!page.Equals(1))
    {
    %>
    <a href="/Member/CarOrder?page=1" class="first">首页</a>
    <a href="/Member/CarOrder?page=<%:(page - 1) %>" class="prev">上一页</a>
    <%    
    }
    else
    {
...
<%for(int i = 1; i <= Npage; i++)
{
    if (page > 10)
    {
        for (int m = 6; m >= 1; m--)
        {
            %><a href="/Member/CarOrder?page=<%:(page - m) %>"><%:(page - m) %></a><%  
        }
        for (int n = 0; n <= 6; n++)
        {
            if (n.Equals(0))
            ...
            else
            {
                if ((page + n) < Npage)
                {
                    %><a href="/Member/CarOrder?page=<%:(page +n) %>"><%:(page + n) %></a><%
                }
            }
        }
        break;
    ...
    else
    {
        if (i <= 10)
        {
            ...
            {
                %><a href="/Member/CarOrder?page=<%:i %>"><%:i %></a><%
            }
        }   
    }
    %>
...
{
    %>
    <a href="/Member/CarOrder?page=<%:page+1 %>">下一页</a>
    <a href="/Member/CarOrder?page=<%:Npage %>">尾页</a>
    <%                
}
%>
</div>
